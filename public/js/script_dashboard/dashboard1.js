document.addEventListener("DOMContentLoaded", function () {
  ...
  // Create an array to hold the series data
    const seriesData = [];

    // Loop through each year in groupedByYear
    for (const year in groupedByYear)
    {
        if (groupedByYear.hasOwnProperty(year))
            {
            // Extract net_total values for the current year
            const data = groupedByYear[year].map(month => parseFloat(month.net_total));
            // Add the series data for the current year
            seriesData.push({
            name: year,
            data: data
            });
  ...
  var chart =
  {
    ...
    colors: ["var(--bs-danger)", "var(--bs-secondary)", "var(--bs-primary)", "var(--bs-success)"],
    plotOptions: {},
    dataLabels: {
      enabled: false,
    },
    ...
    xaxis: {
      ...
      categories: ["Jan", "Fév", "Mar", "Avr", "Mai", "Jui", "Juil", "Aout", "Sep", "Oct", "Nov", "Déc"],
    },
    markers: {
      strokeColor: [
        "var(--bs-danger)",
